...
def get_product_metrics():
    ...
    total_cost_price = sum(
        product.quantity * product.cost_price for product in products
    )
    total_selling_price = sum(
        product.quantity * product.selling_price for product in products
    )
...
def get_sales_metrics():
    ...
    total_product_sold = (
        Outflow.objects.aggregate(Sum("quantity"))["quantity__sum"] or 0
    )
...
def get_daily_sales_data():
    ...
    dates = [
        str(today - timezone.timedelta(days=i)) for i in range(6, -1, -1)
    ]  # Captura as últimas seis data anteriores ao dia de hoje, em ordem decrescente
    values = list()
